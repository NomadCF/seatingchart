...
async function ready(page) {
  ...
  await expect.poll(() => page.evaluate(() => Boolean(window.InterfaceAssistantAuditV721))).toBe(true);
}
...
async function seedClass(page) {
  await page.evaluate(() => {
    ...
    state.groups = [normalizeGroupRecord({
      id:'group-reading',
      name:'Reading Partners',
      type:'together',
      priority:7,
      studentIds:['student-ada','student-grace'],
      anchorSeats:[],
      zoneId:''
    }, 0)];
...
const naturalRequests = [
  'Put Ada beside Grace',
  'Seat Ada next to Grace',
  "Don't put Noah by Eli",
  'Move Maya closer to the front',
  'Find a place for Noah to sit',
  'Why did you put Grace there?',
  'Arrange the desks in rows',
  'Put the desks into pods',
  'Set up an exam layout with 6 ft spacing',
  'Set up a station rotation with 3 teams',
  'Can you fix this seating chart?',
...
test('Planner Assistant normalizes common teacher language into supported planner actions', async ({ page }) => {
  ...
  const results = await page.evaluate(requests => requests.map(request => {
    const normalized = window.InterfaceAssistantAuditV721.normalizeAssistantCommand(request);
    const result = window.PlannerAssistantV710.interpret(normalized);
    return { request, normalized, intent:result.intent, blockers:result.blockers || [] };
  }), naturalRequests);
...
test('student named Ada is not mistaken for an accessibility requirement', async ({ page }) => {
  ...
  const preview = await page.evaluate(async () => {
    window.InterfaceAssistantAuditV721.open('Seat Ada next to Grace');
    await new Promise(resolve => setTimeout(resolve, 20));
    const current = window.PlannerAssistantV710.currentPreview();
    return {
      intent:current?.intent,
      relation:current?.parameters?.relation,
      requirementChanges:current?.parameters?.requirementChanges || {},
      command:current?.command
    };
  });
  expect(preview.intent).toBe('rule_changes');
  expect(preview.relation).toBe('together');
  expect(preview.requirementChanges.ada).toBeUndefined();
  expect(preview.command).toContain('Lovelace');
});

test('unrecognized requests become guidance instead of a dead-end parser error', async ({ page }) => {
  await ready(page);
  await seedClass(page);
  ...
  await expect(page.locator('#plannerAssistantV721PreviewGuide')).toBeVisible();
});
...
test('Assistant has expanded, compact, and fully hidden workspace states with central recovery controls', async ({ page }) => {
  ...
  await page.evaluate(() => window.InterfaceAssistantAuditV721.setMode('expanded', { announce:false }));
  await expect(page.locator('#plannerAssistantV721Dock')).toBeVisible();
  await expect(page.locator('#plannerAssistantV710Dock')).toBeHidden();
  await expect(page.locator('#plannerAssistantV710Restore')).toBeHidden();

  await page.evaluate(() => window.InterfaceAssistantAuditV721.setMode('compact', { announce:false }));
  await expect(page.locator('#plannerAssistantV721Dock')).toBeHidden();
  await expect(page.locator('#plannerAssistantV721Compact')).toBeVisible();

  await page.evaluate(() => window.InterfaceAssistantAuditV721.setMode('hidden', { announce:false }));
  await expect(page.locator('#plannerAssistantV721Dock')).toBeHidden();
  await expect(page.locator('#plannerAssistantV721Compact')).toBeHidden();
  await expect(page.locator('#plannerAssistantV721Settings')).toHaveCount(1);
  await expect(page.locator('#plannerAssistantV721HelpCard')).toHaveCount(1);
  ...
  await expect(page.locator('#plannerAssistantV710HideDockBtn')).toHaveText('Hide Assistant');
  await page.locator('#plannerAssistantV710HideDockBtn').click();
  await expect.poll(() => page.evaluate(() => window.InterfaceAssistantAuditV721.mode())).toBe('hidden');
  await expect(page.locator('#plannerAssistantV710Modal')).not.toHaveClass(/\bshow\b/);
});
...
test('UI audit removes repeated Guide me controls and keeps the workspace within the viewport', async ({ page }) => {
  ...
  await page.evaluate(() => {
    ...
    for (let index = 0; index < 3; index += 1) {
      ...
      button.className = 'guided-context-button';
  ...
  expect(audit.contextualGuideCount).toBe(0);
  expect(audit.legacyDockVisible).toBe(false);
  expect(audit.legacyRestoreVisible).toBe(false);
